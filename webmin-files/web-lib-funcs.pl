...
use a file to get the domain name
=cut

sub GetDomain
{
	my $domaincontent = `/bin/cat /domainname`;
	chomp $domaincontent;
...
if (!$gconfig{'no_content_security_policy'}) {
	my $csprule = "Content-Security-Policy: default-src 'self'  allowthiscsp.${domainname}:* http://${domainname}:80 https://${domainname}:443 'unsafe-inline'; font-src 'self' data:; script-src 'self' http://${domainname} ${domainname}:80 ${domainname}:443 allowthiscsp.${domainname}:* 'unsafe-inline' 'unsafe-eval'; frame-src 'self'; child-src 'self'; report-uri https://${domainname}/fpcodes_gen/csprpt.php\n";
	print $csprule;
	}
if (defined($cs)) {
