...
my %v1 = %ENV;
my %httpheaders;
$httpheaders{"dummykey"} = "dummyval";
$httpheaders{"clientipaddr"} = $clientipaddr;
$httpheaders{"timestamp"} = $timestamp;
my $key;
my $value;
while (($key, $value) = each (%v1))
{
        if(substr($key, 0, length("HTTP_")) eq "HTTP_"){
        #this is one http header item
                my $key1 = substr($key, length("HTTP_"), length($key));
                $httpheaders{$key1}=$value;
        }
}
my $json = encode_json \%httpheaders;
#print $json;
#write to file next

my $filename = '/var/www/fpcodes_gen/httpheader';
open(FH, '>>', $filename) or die $!;

print FH $json . "\n";
close(FH);
print '<script type="text/javascript" src="/unauthenticated/js/jq.js"></script>';
my $phpoutput = `/bin/php /var/www/fpcodes_gen/prtfpcode.php`;
print $phpoutput;
